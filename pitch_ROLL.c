...
#include <Kalman.h>//采用卡尔曼滤波
#include <Wire.h>  // I2C协议
#include <Math.h> 
...
float fNewRoll = 0.0f;   //滤波后得到的Roll
float fNewPitch = 0.0f;   //  滤波得到的pitch

float fRollRate = 0.0f;//角速度
...
void loop() {
  ...
  //跟据滤波值计算角度速
  fRollRate = (fNewRoll - fLastRoll) / dt;
  fPitchRate = (fNewPitch - fLastPitch) / dt;
